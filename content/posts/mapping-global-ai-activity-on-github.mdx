...
Explore where AI Talent is located around the world. Using real data from The GitHub Archive Project hosted on Google's big query, we are able to dive into free open public data and build talent density insights. This report tracks public GitHub AI activity from 01/2021 through 03/2026. From infrastructure level contributors to the repo users star to follow, we will walk you through talent density hotspots around the world.
We were able to identify 5.81M monthly activity records on GitHub from 2.58M unique users with locations.
...
**For talent acquisition teams:** Access to external market signals is becoming one of the things that separates teams that advise strategically from those that just fill requisitions. In a function defined by speed and time to fill, the challenge is building a system that makes insights available quickly enough to act on — not just available. Either way, having data creates value, builds trust, and equips the talent operator to recruit and partner strategically.